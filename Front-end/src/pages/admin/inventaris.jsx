            ...
            <tbody>
              {orders.map((productModel, index) => (
                ...
                >
                  ...
                  <td className="py-4 px-6">{productModel.items[0].name},{productModel.items}</td>
                  <td className="py-4 px-6">{productModel.categorie}</td>
                  <td className="py-4 px-6">{productModel.beschikbaar.quantity}</td>
                  <td className="py-4 px-6">{productModel.uitgeleend.quantity}</td>
                  <td className="py-4 px-6">{productModel.gereserveerd.quantity}</td>
                  <td className="py-4 px-6">{productModel.gepauzeerd.quantity}</td>
                  <td className="py-4 px-6">
                    <div>
                      <Link
                        to={`/admin/inventaris/wijzigen/${order.id}`}
                        className="bg-red-500 hover:bg-red-900 text-white py-2 px-4 rounded-lg"
                      >
                        wijzig
                      </Link>
                      <Link
                        to={`/admin/inventaris/info/${order.id}`}
                        className="bg-red-500 hover:bg-red-900 text-white py-2 px-4 rounded-lg"
                      >
                        info
                      </Link>
